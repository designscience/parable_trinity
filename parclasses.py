...
class ValvePort_Recorder(ValvePort):
    """This special case descendeant of ValvePort plays music and records any output generated by the user, storing it
     with media-relative times in one or more ControlList objects. Each of these objects may be considered a 'layer'.
     When one or more layers are finished the programs commits these, which combines the ControlList layers into one
    ...
    def __init__(self, channels=24, channelsperbank=6, media_path='./', kill_callback=None):
        ...
        self.layer_recorded = False  # Dirty flag that indicates that the layer has been written to

        self.recording_start = 0.0  # System time recording started
        self.media_loaded = None  # Name of media file
    ...
    def stop(self):
        ...
        self.player.stop()  # will call on_completion()
        return self.layer_recorded
    ...
    def reject(self):
        ...
        if self.layer_recorded:
            self.layers[self.current_layer] = ControlList()
            self.layer_recorded = False
        self.player.rewind()
    ...
    def _init_layers(self):
        """Initilizes this object with one layer, optionally loading that first layer from a file"""
